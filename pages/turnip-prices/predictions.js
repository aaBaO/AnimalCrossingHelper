...
function* generate_pattern_0_with_lengths(given_prices, high_phase_1_len, dec_phase_1_len, high_phase_2_len, dec_phase_2_len, high_phase_3_len) {
  ...
  */
  ...
  for (var i = 2; i < 2 + high_phase_1_len; i++) {
    var min_pred = Math.floor(0.9 * buy_price);
    var max_pred = Math.ceil(1.4 * buy_price);
...
function* generate_pattern_1_with_peak(given_prices, peak_start) {
  ...
  */
  ...
  var min_randoms = [0.9, 1.4, 2.0, 1.4, 0.9, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4]
  var max_randoms = [1.4, 2.0, 6.0, 2.0, 1.4, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9]
  for (var i = peak_start; i < 14; i++) {
    var min_pred = Math.floor(min_randoms[i - peak_start] * buy_price);
    var max_pred = Math.ceil(max_randoms[i - peak_start] * buy_price);
...
function* generate_pattern_3_with_peak(given_prices, peak_start) {
  ...
  */
  ...
  // Main spike 1
  var min_pred = Math.floor(1.4 * buy_price) - 1;
  var max_pred = Math.ceil(2.0 * buy_price) - 1;
...
function analyze_possibilities(sell_prices, first_buy, previous_pattern) {
  ...
  var generated_possibilities = get_probabilities(generated_possibilities, previous_pattern);
